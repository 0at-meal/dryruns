...
| `item_description_is_missing` | Binary missingness feature | Preserve as an explicit signal in sanitized modeling data |
...
## U2. Missing `brand_name`

- **Finding:** ~42.6-42.7% missing in both train and test.
- **Current decision:** Represent missing brand with `__MISSING__` in sanitized data.
- **Later decision:** Test whether a separate brand-missing indicator adds predictive value.

## U3. Category hierarchy representation

- **Finding:** `category_name` is overwhelmingly three levels deep, with legitimate but rare levels 4-5.
- **Current decision:** Preserve all available hierarchy information.
- **Later decision:** Determine whether the model benefits from the original path, individual levels, or both.

## U4. Near-duplicate / repeated listing structure

- **Finding:** Large normalized duplicate groups exist in both text fields; normalized validation/train overlap is non-trivial.
...
- **Current decision:** Raw values remain unchanged; lowercasing is used in the current TF-IDF preparation.
- **Later decision:** Determine whether additional normalization improves validation performance.
...
- **Current decision:** Treat the placeholder as absence of useful description during preprocessing and retain `item_description_is_missing`.
- **Later decision:** Confirm whether retaining the placeholder as a token provides incremental value for any specific text model.

## U9. Target-Statistic Encoding

- **Finding:** Properly leakage-safe OOF `category_log_mean`, `brand_log_mean`, and `category_brand_log_mean` features produced RMSLE 0.477509 versus 0.475911 for the current best Ridge model.
- **Current decision:** Reject these features from the active Ridge feature set.
- **Reason:** They worsened validation RMSLE by 0.001598 and increased runtime.
- **Later decision:** Do not add further target-encoding complexity unless a later experiment provides a concrete reason to revisit it.
...
4. Initial Baseline
   -> Complete
   -> Reference RMSLE: 0.593385
   -> Reference baseline: category_name + brand_name median
...
5. Feature Engineering & Construction
   ...
   -> 5.6 Complete
   -> 5.7 Complete
   -> Candidate feature families ready for benchmarking
...
6. Iterative Model Benchmarking
   ...
   -> 6.7 Complete
   -> 6.8 Complete
   -> Current best: TF-IDF + Structured + Ridge
   -> Current best RMSLE: 0.475911
```
